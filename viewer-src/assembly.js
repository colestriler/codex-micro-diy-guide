...
allMeshes.length=0;for(const e of entries){if(!MODEL_PARTS[e.id])throw new Error('Missing catalog mapping: '+e.id);e.obj.traverse(m=>{if(m.isMesh){m.userData.partKey=m.userData.partKey||MODEL_PARTS[e.id];m.userData.id=e.id;m.userData.baseColor=m.material.color.clone();allMeshes.push(m);}});}
