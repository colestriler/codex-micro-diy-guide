...
renderer.setClearColor(0xe9eee3,1);renderer.outputColorSpace=THREE.SRGBColorSpace;
renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;
viewer.prepend(renderer.domElement);renderer.domElement.setAttribute('role','img');renderer.domElement.setAttribute('aria-label','Rotatable CAD assembly of the DIY Micro. Use the stage controls for a text description of every assembly step.');
...
const copper=cylinder(13,.15,C[0],R[3],24.1,'#b57448');byId.get('touch').obj.add(copper);
...
const lightEmitters=[];
function ledBoard(){const g=new THREE.Group();g.add(box(9.1,9.1,1,0,0,0,'#2c6852'));g.add(box(5,5,1.3,0,0,1,'#eeeecc'));const die=box(3.8,3.8,.8,0,0,2.3,'#c4d9b7');die.material.emissive.set('#71b684');die.material.emissiveIntensity=.25;g.add(die);lightEmitters.push(die);return g;}
...
const support=new THREE.Group();support.add(box(25,20,1.6,0,-35,4,'#897950'));support.add(box(7,19,3,0,-35,5.6,'#283129'));support.add(cylinder(5,5,8,-30,5.6,'#556555'));
...
for(const x of [-20,20])for(const y of [-20,20]){const w=ring(24,8.4,2,x,y,-2.2,'#9caa99');foot.add(w);}
for(const [x,y] of [[0,34],[0,-34],[34,0],[-34,0]]){const r=cylinder(9,1.2,x,y,-6.2,'#26352b');foot.add(r);}
...
allMeshes.length=0;for(const e of entries)e.obj.traverse(m=>{if(m.isMesh){m.userData.id=e.id;m.userData.baseColor=m.material.color.clone();allMeshes.push(m);}});
...
let progress=0,displayStage=-1,playing=false,animation=null,holdUntil=0,lastTime=0;
let theta=.78,elevation=.48,zoom=1,span=160,target=new THREE.Vector3(8,0,73),selectedId=null;
let annotationNodes=[];
...
function paintParts(t){topPack.position.z=70*(1-done(t,7));for(const e of entries){const d=e.move(t);e.obj.position.set(e.base.x+d[0],e.base.y+d[1],e.base.z+d[2]);}cable.visible=t>10;loom.visible=t>=5.98;wirePaths(topPack.position.z);const colorStates=['#e8eedf','#649dff','#76d794','#ffc264','#ec7479','#bac9b8'];agents.forEach(([x,y],i)=>{const id='cap'+(i<2?'0'+(i+1):'1'+(i-2));const obj=byId.get(id).obj;obj.traverse(m=>{if(m.isMesh){m.material.color.copy(m.userData.baseColor);if(t>10.6)m.material.color.lerp(new THREE.Color(colorStates[i]),.8);m.material.emissive.set(t>10.6?colorStates[i]:'#000000');m.material.emissiveIntensity=t>10.6?(i===5?.02:.22):0;}});});lightEmitters.forEach(m=>m.material.emissiveIntensity=t>10.6?.65:.15);[shell,foot].forEach(part=>part.traverse(m=>{if(m.isMesh){m.material.transparent=$('xray').checked;m.material.opacity=$('xray').checked?.2:1;m.material.depthWrite=!$('xray').checked;}}));if(selectedId){byId.get(selectedId).obj.traverse(m=>{if(m.isMesh){m.material.emissive.set('#93bb63');m.material.emissiveIntensity=.26;}});}}
...
function clearSelection(){if(selectedId)byId.get(selectedId).obj.traverse(m=>{if(m.isMesh){m.material.emissive.set('#000000');m.material.emissiveIntensity=0;}});selectedId=null;}
renderer.domElement.addEventListener('pointerup',e=>{if(moved<5&&pointers.size===1){const r=renderer.domElement.getBoundingClientRect();raycaster.setFromCamera(new THREE.Vector2((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1),camera);const hits=raycaster.intersectObjects(allMeshes).filter(h=>{let o=h.object;while(o){if(!o.visible)return false;o=o.parent;}return true;});clearSelection();if(hits.length){selectedId=hits[0].object.userData.id;const p=byId.get(selectedId);$('selected').textContent=p.name+' · '+p.kind;}else $('selected').textContent='Select any visible part to identify it.';}pointers.delete(e.pointerId);});
...
function tick(now){const dt=now-lastTime;lastTime=now;
 ...
 $('progress').value=progress;paintParts(progress);root.updateMatrixWorld(true);frameCamera();positionLabels();renderer.render(scene,camera);requestAnimationFrame(tick);
}
stageText(0);paintParts(0);requestAnimationFrame(tick);
})();
