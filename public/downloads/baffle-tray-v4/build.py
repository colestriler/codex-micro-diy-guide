"""V4: 10 x 4 mm LED access openings; two-layer LED tray using existing M3x8 screws and M3x4 inserts. Units mm. Physical fit pending.
...
for x,y in centers:
 ...
 # Restore the original 10 x 4 mm access envelope and -5 mm offset.
 # Rounded corners remain; a led can pass edgewise before the upper plate is fitted.
 tray=tray.cut(rr(P['led_access_width_mm'],P['led_access_depth_mm'],3,.4,x,y+P['led_access_y_offset_mm'],base-.1))
...
results={'revision':'Two-layer baffle tray v4; 10 x 4 mm LED access; physical fit pending','mount_centers_mm':mounts,'cup_centers_mm':centers}
# Restrict the revision to the floor openings; preserve the upper plate.
previous_tray=cq.importers.importStep(str(ROOT/'references/previous-tray-v3.step'))
previous_plate=cq.importers.importStep(str(ROOT/'references/previous-upper-plate.step'))
changed=tray.cut(previous_tray).union(previous_tray.cut(tray))
for x,y in centers:
 changed=changed.cut(box(10.2,5.2,3.2,x,y-5.5,base-.1))
results['changes_outside_LED_access_regions_mm3']=sum(s.Volume() for s in changed.solids().vals())
assert results['changes_outside_LED_access_regions_mm3']<.001,'Unexpected tray change outside openings'
results['upper_plate_change_mm3']=sum(s.Volume() for difference in [plate.cut(previous_plate),previous_plate.cut(plate)] for s in difference.solids().vals())
assert results['upper_plate_change_mm3']<.001,'Upper plate changed'
...
# Validate the access opening with a nominal un-wired LED envelope.
# Move edge-first through the floor, shift inward, turn flat, then seat it.
# No top plate/switches are present during this insertion; solder and wires
# are deliberately NOT represented by this bounding-box check.
results['led_access_opening_mm']=[P['led_access_width_mm'],P['led_access_depth_mm']]
results['led_access_y_offset_mm']=P['led_access_y_offset_mm']
results['nominal_LED_edgewise_clearance_total_mm']=[P['led_access_width_mm']-9.1,P['led_access_depth_mm']-3.1]
for x,y in centers:
 for z in [base-9.2,base-4.5,base,12.45]:
  assert vol(tray,box(9.1,3.1,9.1,x,y+P['led_access_y_offset_mm'],z))<.001,'LED edgewise entry blocked'
 for dy in [-5,-4,-3,-2,-1,0]:
  assert vol(tray,box(9.1,3.1,9.1,x,y+dy,12.45))<.001,'LED inward movement blocked'
 for angle in range(0,91,10):
  led_edge=box(9.1,3.1,9.1,z=-4.55).rotate((0,0,0),(1,0,0),angle).translate((x,y,17))
  assert vol(tray,led_edge)<.001,'LED rotation blocked'
 for z in [15.45,14,12,11]:
  assert vol(tray,box(9.1,9.1,3.1,x,y,z))<.001,'LED seating blocked'
 # A continuous strip of floor remains beneath the far half of the LED.
 support=box(8,5,.7,x,y+1,base)
 assert abs(vol(tray,support)-support.val().Volume())<.001,'LED support floor missing'
results['nominal_unwired_LED_insertion_path_clear']=True
results['LED_insertion_note']='CAD envelope only: 9.1 x 9.1 x 3.1 mm LED, upper plate and switches removed. Solder, insulation and wire bends are not modeled; test the actual wired LED on the sample without forcing it.'
...
fig.suptitle('Larger LED openings. Same screws and inserts.',fontsize=19)
fig.text(.5,.04,'V4 · 10 x 4 mm bottom openings · black PETG lower tray · M3 x 8 screws + M3 x 4 heat-set inserts · physical fit pending',ha='center',fontsize=10)
