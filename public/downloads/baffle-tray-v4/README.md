# Two-layer baffle tray v4 — 10 × 4 mm LED openings

Each compartment now has a **10 × 4 mm bottom opening**, enlarged from v3's 6 × 2.2 mm slot to match the original individual baffle opening size. The opening is centered 5 mm toward the wire side of the LED seat, matching the original offset and leaving a 1.15 mm strip before the inner wall. Corners retain a 0.4 mm radius.

The upper plate, screw posts, heat-set inserts and 9.7 × 9.7 mm LED recesses are unchanged. Reprint only the lower sample or lower tray that you need. Use the same M3 × 8 mm screws and Adafruit M3 × 4 mm inserts. Physical fit remains unverified.

## Feeding a light through

Unplug USB and remove the upper plate so the compartment is open. Turn the LED **edge-first** through the 10 × 4 mm opening, then move it toward the center and turn it flat into its shallow seat. Do this before closing the upper plate over the LEDs. Test your already-soldered light on the small sample before installing inserts; remove the light before heating any insert. During the LED fit check, feed the wires gently and stop if a solder joint catches. Do not pull on the wires to force the LED through.

The CAD checks a nominal 9.1 × 9.1 × 3.1 mm LED body along an edge-first insertion, inward movement, rotation and seating path. Its edge-on envelope has 0.9 mm total clearance in both slot dimensions (0.45 mm per side when centered). Solder, insulation, wire bends and printer tolerances use that clearance and are not modeled. The opening is not large enough to drop this LED through flat. A physical fit check is still required.
...
LED pockets locate but do not clamp boards. Retain them with one layer of 3M 9474LE polyester-backed mounting tape (0.17 mm excluding liners), within the 0.2 mm adhesive allowance. Cut small pieces; keep LED faces and wire openings clear. Dry-fit and test first: this is strong permanent adhesive. Product: https://www.amazon.com/dp/B0CLJ3RK2B LED leads use floor slots; matrix leads use wall notches. Leave about 20 mm lowering travel in the wires and keep all metal pads isolated. No hot-swap or electrical disconnect is added.

`build.py` checks the nominal LED insertion path and verifies that changes from v3 are confined to the opening regions, with zero upper-plate geometry change. It exports four single-solid watertight printable meshes, checks screw/insert fit, nominal engagement, tip clearance, insertion-tip access, case and electronics envelopes, and tray removal past switches. It does not validate real soldering or physical strength. STEP uses assembled coordinates; STL is print-oriented. Reference assembly is not printable. Viewer and preview use the same CAD.

Run `python build.py` with Python 3.12 and `requirements.txt`. Test both the baffle sample and joystick sample before the full plate. The older main keyboard animation remains a legacy illustration. See this folder's interactive guide for v4.
