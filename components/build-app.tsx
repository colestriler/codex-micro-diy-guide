  ...
  return (
    <main id="top">
      ...
      <section id="assembly" className="build-section" aria-labelledby="heading-assembly">
          ...
          <p>The baffles in this animation are the older adhesive design. <a href="/downloads/baffle-tray-v4/guide.html">See the new two-layer tray and its interactive CAD view</a> before printing.</p>
      ...
      <section id="files" className="build-section" aria-labelledby="heading-files">
          ...
          <div className="print-summary" id="current-cad"><h3>Current STEP files</h3><p>The latest set combines the flat joystick mount v3 with the heat-set baffle tray v4, now with 10 × 4 mm LED access openings. The full upper plate includes both the joystick supports and the four tray screw holes. The upper plate and upper fit sample are unchanged from the two-layer v2 design.</p><div className="file-shortcuts"><a href="/downloads/current-cad/Codex-Micro-current-STEP.zip" download>Download current STEP set ↓</a><a href="/downloads/step/02_two_layer_top_plate_WAIT.step" download>Upper plate STEP — WAIT ↓</a><a href="/downloads/step/07_heatset_six_light_tray_v4_WAIT.step" download>Heat-set tray STEP — WAIT ↓</a><a href="/downloads/baffle-tray-v4/files/reference_assembly_NOT_PRINTABLE.step" download>Tray assembly STEP — view only ↓</a></div><p>The ZIP contains 13 current part and fit-sample STEP files, plus the partial tray assembly for reference. It excludes retired mounts, shims and baffles. Use the three small fit samples before printing the full-size pair; their individual STL and STEP links are below.</p></div>
          ...
          <div className="print-summary"><h3>Tray v4: larger openings for the LEDs</h3><p>Each compartment now has a <b>10 × 4 mm bottom opening</b>, enlarged from 6 × 2.2 mm to match the original baffle opening size. Only the lower piece needs reprinting; reuse your two-layer upper plate or upper sample. Feed the LED edge-first with the upper plate off, then turn it flat into its seat. Test the soldered wires on the sample first.</p><p>The upper plate holds the switches. A rigid lower tray contains all six light compartments. Your existing M3 × 8 screws join the layers using heat-set inserts in the tops of the four tray posts. No loose nuts or flexible clips.</p><p>Print the two-key upper and lower samples first. Use two existing M3 × 8 screws and two M3 × 4 inserts for the sample. The screws are reusable; the inserts stay in the sample. The v2 upper sample is unchanged and can be reused. Use PETG at 100% scale, 0.15 mm layers and supports off for these samples. Cool the inserts fully before assembly. Heat-set grip, wiring clearance and physical strength still need testing.</p><p><a href="/downloads/stl/07_two_layer_top_sample.stl" download>1. Upper sample STL ↓</a> · <a href="/downloads/step/07_two_layer_top_sample.step" download>Upper sample STEP ↓</a> · <a href="/downloads/stl/07_heatset_baffle_sample_v4.stl" download>2. Lower sample STL ↓</a> · <a href="/downloads/step/07_heatset_baffle_sample_v4.step" download>Lower sample STEP ↓</a> · <a href="/downloads/baffle-tray-v4/guide.html" target="_blank" rel="noopener noreferrer">Interactive CAD, hardware and assembly ↗</a></p><img src="/downloads/baffle-tray-v4/preview.png" alt="Two-layer CAD: upper switch plate, removable six-light tray and small two-key test pair" style={{width:"100%",height:"auto"}}/><p>The full-size pair below is marked WAIT. Finish the baffle and joystick fit tests first. The lower tray is black PETG and prints cups up; the full upper plate has its own orientation notes.</p></div>
          <div className="print-summary"><h3>New: flat joystick mount v3</h3><p>A flat plate with two raised tab supports and exposed nuts underneath. Print just the small test corner first—one piece, with no nut boxes or separate shims.</p><p>Existing PETG, 100% scale, 0.10 mm layers. Flat side down, raised supports up. Supports OFF for the test corner. Use the same M1.6 × 8 mm screws and M1.6 nuts; hold the exposed nuts with small pliers.</p><p><a href="/downloads/stl/04_flat_joystick_corner_v3_PRINT_FIRST.stl" download>Download the ONE v3 test STL ↓</a> · <a href="/downloads/step/04_flat_joystick_corner_v3_PRINT_FIRST.step" download>Test corner STEP ↓</a> · <a href="/downloads/joystick-flat-v3/guide.html" target="_blank" rel="noopener noreferrer">Print and fit guide ↗</a></p><img src="/downloads/joystick-flat-v3/preview.png" alt="Flat joystick mount with raised tab seats and accessible nuts underneath" style={{width:"100%",height:"auto"}}/><p>Wait for the corner fit check before printing the full plate. CAD clearance checks pass; physical fit, strength and full thumb-cap movement remain unverified. The full-keyboard animation and original assembly STEP still show the old mount.</p><p><a href="/downloads/joystick-flat-v3/flat-mount-v3-kit.zip" download>Complete v3 CAD/source kit ↓</a> · <a href="/downloads/joystick-integrated-v2/guide.html">Previous recessed mount v2 (archive)</a></p></div>
